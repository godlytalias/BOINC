...
{
char file_name[40];
sprintf(file_name,"../graphiso/prob_prop_%d_%d",graph_id,initstate);
...
for(long i=0;i<node;i++){
         ...
         map_g[0][i].map_ver=i;
for(long j=0;j<node;j++)
	fscanf(infile,"%f",&g1[i][j]);}

for(long i=0;i<node;i++){
         g2[i]=new float[node];
         map_g[1][i].map_ver=i;
for(long j=0;j<node;j++)
fscanf(infile,"%f",&g2[i][j]);}
fclose(infile);

FILE *read;
  sprintf(filename,"../graphiso/prob_prop_%d_%ld",0,w_node);  
  read=fopen(filename,"r");
  if(!read)
